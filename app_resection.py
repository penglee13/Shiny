...
csv_path = "training_resection.csv"
...
feature_cols = [
    ...
    'GENDER_M',
    'Number_Brainmet_2',
    'BM_Location_Supratentorial',
    'BM_Location_Both',
    'Primary_cancer_response_at_intervention_Progressing',
    'PRE_KPS_group_gt_80',
    'SYMPTOMS_BEFORE_SURGERY_Yes'
]
...
input_widgets = [
    ...
    ui.input_numeric('PRE_Vol1_total', 'Pre intervention total hyperintensity tumor volume', 10, min=0, max=1000),
    ui.input_select('GENDER_M', 'Gender', {'1': 'Male', '0': 'Female'}, selected='1'),
    ui.input_select('Number_Brainmet_2', 'Number of brain metastasis', {'1': '1', '2': '2'}, selected='1'),
...
def get_new_patient(input):
    training = pd.read_csv('training_resection.csv')
    X_train = training.drop(["ID", "OS.time", "OS"], axis=1)
    new_patient = X_train.iloc[[0]].copy()

    # Update with actual CSV column names
    if 'Age at BM Diagnosis' in new_patient.columns:
        new_patient['Age at BM Diagnosis'] = float(input.Age_at_BM_diagnosis())
    if 'Pre Intervention Total Hyperintensity Tumor Volume' in new_patient.columns:
        new_patient['Pre Intervention Total Hyperintensity Tumor Volume'] = float(input.PRE_Vol1_total())
    if 'Gender Male (vs Female)' in new_patient.columns:
        new_patient['Gender Male (vs Female)'] = int(input.GENDER_M())
    if 'Number of BM 2 (vs 1)' in new_patient.columns:
        new_patient['Number of BM 2 (vs 1)'] = 1 if input.Number_Brainmet_2() == '2' else 0
...
def server(input, output, session):
    ...
    def current_obs():
        return {
            ...
            'PRE_Vol1_total': input.PRE_Vol1_total(),
            'GENDER_M': input.GENDER_M(),
            'Number_Brainmet_2': input.Number_Brainmet_2(),
    ...
    def reset_inputs():
        ...
        session.send_input_message('PRE_Vol1_total', 10)
        session.send_input_message('GENDER_M', '1')
        session.send_input_message('Number_Brainmet_2', '1')
    ...
    @reactive.event(
        ...
        input.PRE_Vol1_total,
        input.GENDER_M,
        input.Number_Brainmet_2,
        input.bm_location,
